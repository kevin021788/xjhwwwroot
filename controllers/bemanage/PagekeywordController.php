...
use app\models\PagekeywordSearch;
...
/**
 * PagekeywordController implements the CRUD actions for Pagekeyword model.
 */
class PagekeywordController extends Controller
{
    ...
    /**
     * Lists all Pagekeyword models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new PagekeywordSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        //可编辑列操作   保存数据   开始
        if (Yii::$app->request->post('hasEditable')) {
            $controller = ucfirst(replaceBackend(Yii::$app->controller->id));
            $id = Yii::$app->request->post('editableKey');
            $model = Pagekeyword::findOne(['id' => $id]);
    ...
    /**
     * Displays a single Pagekeyword model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new Pagekeyword model.
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if(isset($image)) {
                ...
                $model->imgUrl = Yii::$app->params['uploadPagekeywordPath'] . date('YmdHis', time()).mt_rand(1000,9999).".{$ext}";
                $path = '.' . $model->imgUrl;
            }
            if($model->save()){
    ...
    /**
     * Updates an existing Pagekeyword model.
    ...
    /**
     * Deletes an existing Pagekeyword model.
    ...
    public function actionDeleteAll()
    {
        ...
        $rs = Pagekeyword::deleteAll(['id'=>$ids]);
    ...
    public function actionChangeStatus()
    {
        ...
        $rs = Pagekeyword::updateAll(['status' => $status],['in','id',$ids]);
    ...
    /**
     * Finds the Pagekeyword model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Pagekeyword the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Pagekeyword::findOne($id)) !== null) {
            return $model;
        } else {
